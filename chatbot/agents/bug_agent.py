from __future__ import annotations

import json
import sys
from pathlib import Path

from langchain_core.messages import HumanMessage, SystemMessage
from langchain_openai import ChatOpenAI

ROOT_DIR = Path(__file__).resolve().parents[2]
root_str = str(ROOT_DIR)
if root_str not in sys.path:
    sys.path.insert(0, root_str)

from chatbot.schemas import ChatbotState
from chatbot.tools.db_tools import (
    read_gacha_logs,
    read_item_delivery_logs,
    write_answer_draft,
    write_evidence_docs,
    write_ticket_analysis,
)
from config import settings

_SYSTEM_PROMPT = """당신은 게임 CS 인게임 버그 전문 Agent입니다.
아이템 지급 로그와 가챠 기록을 분석하여 버그 여부를 판단하고,
재현 조건 및 운영팀 에스컬레이션 필요 여부를 안내하세요."""


def _get_llm() -> ChatOpenAI:
    return ChatOpenAI(
        model=settings.openai_model,
        api_key=settings.openai_api_key,
        temperature=0.3,
    )


def bug_agent_node(state: ChatbotState) -> dict:
    ticket_id = state["ticket_id"]
    account_id = state.get("account_id") or 0
    user_message = state.get("user_message", "")

    delivery_logs = read_item_delivery_logs.invoke({"account_id": account_id})
    gacha_logs = read_gacha_logs.invoke({"account_id": account_id})

    context = f"아이템 지급 로그:\n{delivery_logs}\n\n가챠 로그:\n{gacha_logs}"

    write_ticket_analysis.invoke({
        "payload": {"ticket_id": ticket_id, "category": "인게임버그", "risk_level": "MEDIUM"},
    })

    llm = _get_llm()
    response = llm.invoke([
        SystemMessage(content=_SYSTEM_PROMPT),
        HumanMessage(content=f"문의:\n{user_message}\n\n데이터:\n{context}"),
    ])

    draft_result = write_answer_draft.invoke({
        "payload": {"ticket_id": ticket_id, "content": response.content},
    })
    draft_id = json.loads(draft_result).get("draft_id")

    write_evidence_docs.invoke({
        "payload": {"draft_id": draft_id, "source": "지급 로그/가챠 로그"},
    })

    return {"answer_draft": response.content, "draft_id": draft_id, "retry_count": 0}
