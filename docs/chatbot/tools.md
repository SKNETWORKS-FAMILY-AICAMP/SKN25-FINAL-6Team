...
| `write_voc_feedback(payload)` | VOC 유형, 감정, 원문, topic_keywords를 VOC_DB 성격의 저장소에 적재 |

VOC는 정상적인 고객 의견 접수 흐름입니다. VOC 유형별 접수형 응답은 `answer_draft`와 `final_response`에 저장하지만, `failed_query`에는 저장하지 않습니다.
현재 baseline에서는 VOC 세부 유형/감정/topic_keywords 분류에 LLM을 사용하고, 고객 응답 문구는 유형별 deterministic template을 사용하므로 LLM safety 검사를 생략할 수 있습니다.
