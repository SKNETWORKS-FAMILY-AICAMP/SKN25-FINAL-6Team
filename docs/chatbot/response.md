...
| 파일 | 노드 함수 | 역할 |
|------|-----------|------|
| `final_response.py` | `final_response_node` | `safety_action`에 따라 `final_text` 생성 |
...
현재 baseline은 `final_text`를 state에 남기고, `write_final_response`를 통해 실제 사용자 응답 본문을 `final_response.final_text`에 저장합니다.
