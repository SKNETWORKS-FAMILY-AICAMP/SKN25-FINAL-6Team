"""Build test_documents_chunks from sj_documents.

Default mode is dry-run. Use --apply to rewrite test_documents_chunks and delete
stale test document embeddings for the affected documents.

Use --with-embeddings with --apply to generate and insert:
- test_documents_embeddings_small: text-embedding-3-small, 1536 dimensions
- test_documents_embeddings_large: text-embedding-3-large, 1536 dimensions
Use --with-large-3072 to additionally generate test_documents_embeddings_large_3072.
"""
...
def rechunk_document(document: Document) -> list[Chunk]:
    ...
    if source_type.startswith("hoyoverse_qna"):
        texts = rechunk_qna(document)
    elif source_type in {"hoyoverse_policy", "universe_policy"} and category == "privacy":
        texts = rechunk_privacy(document)
    elif source_type in {"hoyoverse_policy", "universe_policy"} and category == "terms":
        ...
        try:
            with conn.cursor() as cur:
                if document_id or document_ids:
                    ...
                    cur.execute("DELETE FROM test_documents_chunks WHERE document_id = ANY(%s)", (affected_document_ids,))
                else:
                    cur.execute("DELETE FROM test_documents_embeddings")
                    cur.execute("DELETE FROM test_documents_embeddings_small")
                    cur.execute("DELETE FROM test_documents_embeddings_large")
                    cur.execute("DELETE FROM test_documents_chunks")
                ...
                for table, rows in (
                    ("test_documents_embeddings_small", small_embeddings),
                    ("test_documents_embeddings_large", large_embeddings),
                    ("test_documents_embeddings_large_3072", large_3072_embeddings),
                ):
                    if rows:
                        cur.executemany(
                            f"""
...
def main() -> int:
    ...
    parser.add_argument(
        "--with-embeddings",
        action="store_true",
        help="Generate and insert test_documents_embeddings_small and test_documents_embeddings_large.",
    )
    parser.add_argument(
        "--with-large-3072",
        action="store_true",
        help="Also generate and insert test_documents_embeddings_large_3072. Requires --with-embeddings.",
    )
    ...
    if args.apply:
        ...
        if args.with_embeddings:
            small_embeddings = embed_chunk_records(
                chunk_records,
                SMALL_EMBEDDING_MODEL,
                dimensions=EMBEDDING_DIMENSIONS,
                model_label=f"{SMALL_EMBEDDING_MODEL}:1536",
            )
            print(f"generated_small_embeddings={len(small_embeddings)}")
            large_embeddings = embed_chunk_records(
                chunk_records,
                LARGE_EMBEDDING_MODEL,
                dimensions=EMBEDDING_DIMENSIONS,
                model_label=f"{LARGE_EMBEDDING_MODEL}:1536",
            )
            print(f"generated_large_embeddings={len(large_embeddings)}")
            if args.with_large_3072:
                large_3072_embeddings = embed_chunk_records(
                    chunk_records,
                    LARGE_EMBEDDING_MODEL,
                    dimensions=None,
                    model_label=f"{LARGE_EMBEDDING_MODEL}:3072",
                )
            print(f"generated_large_3072_embeddings={len(large_3072_embeddings)}")
        else:
            print("generated_small_embeddings=0")
            print("generated_large_embeddings=0")
            print("generated_large_3072_embeddings=0")
            print("note=test document embedding tables will be cleared for affected documents")
        persist_chunks(
            all_chunks,
            chunk_records,
            small_embeddings,
