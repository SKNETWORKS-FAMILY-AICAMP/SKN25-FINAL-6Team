...
metric_cols[1].metric("평균 독성", "-" if safety["avg_toxicity_score"] is None else f"{safety['avg_toxicity_score']:.2f}")
metric_cols[2].metric("평균 정책위반", "-" if safety["avg_policy_violation_score"] is None else f"{safety['avg_policy_violation_score']:.2f}")
metric_cols[3].metric("평균 사실성", "-" if safety["avg_factuality_score"] is None else f"{safety['avg_factuality_score']:.2f}")

left, right = st.columns(2, gap="large")
with left:
    render_chart_box("분석 위험도", as_bar_chart(summary["analysis_risk_distribution"]))
    render_chart_box("감성 분포", as_bar_chart(summary["sentiment_distribution"]))

with right:
    render_chart_box("인사이트 위험도", as_bar_chart(summary["insight_risk_distribution"]))
    render_chart_box("패턴 위험도", as_bar_chart(summary["pattern_risk_distribution"]))

st.subheader("고위험 후보")
render_data_table(
    as_table_rows(
        summary["high_risk_tickets"],
        ["ticket_id", "title", "status", "category", "risk_level", "sentiment", "routing_target", "pattern_risk_level", "inquiry_created_at"],
    )
)
