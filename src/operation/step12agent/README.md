...
5. BM25 순위와 vector 순위를 RRF로 결합해 최종 chunk를 고른다.
...
BM25(lexical)와 vector similarity 두 검색 결과를 RRF(Reciprocal Rank Fusion)로 결합한다.

- BM25 검색: kiwipiepy 형태소 분석 기반으로 `documents_chunks.chunk_text` 전체 순위 산출
- vector 검색: `CHROMA_PERSIST_DIR` 설정 시 Chroma PersistentClient, 미설정 시 payload 내 embeddings로 in-memory cosine similarity 계산
- RRF fusion: `score = 1/(k + bm25_rank) + 1/(k + vector_rank)` (k = `RRF_K`, 기본값 60)
- vector 검색 실패 시 BM25 단독으로 자동 폴백
...
- `agent.py`: Step12State, step1_node, step2_node, urgent_node, StateGraph(START→step1→[step2|urgent]→END) 구현 완료
