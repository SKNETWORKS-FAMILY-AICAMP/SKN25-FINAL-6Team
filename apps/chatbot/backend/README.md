...
생성된 답변과 근거를 실제 DB에 저장합니다. 저장 로직은 `src/chatbot/repository/` 계층에서 담당하며, 공통 DB 연결은 `src/common/db/connection.py`를 사용합니다.
...
DB 접근 코드는 공통 DB access layer인 `src/common/db/connection.py`를 사용합니다.
...
```bash
PYTHONPATH=.:apps/chatbot/backend \
...
- write 계열 repository는 일부 DB INSERT 구현이 남아 있습니다.
