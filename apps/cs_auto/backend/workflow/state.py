"""State models for the active operation workflow."""
...
HumanDecision = Literal["approved", "regenerate", "edit"]
RiskLevel = Literal["low", "medium", "high", "critical"]


class OperationModel(BaseModel):
    """Base model shared by workflow state payloads."""

    model_config = ConfigDict(extra="allow", populate_by_name=True)


class Ticket(OperationModel):
    """Source ticket loaded from `qa_ticket` and related account tables."""
...
class EvidenceDocument(OperationModel):
    """Retrieved evidence chunk used for drafting and audit trails."""
...
class AnalysisResult(OperationModel):
    """Classification and routing result produced by the intake agent."""
...
class SafetyResult(OperationModel):
    """Review result used before final publication."""
...
class HumanReviewResult(OperationModel):
    """Human review decision and optional edited answer."""
...
class OperationState(OperationModel):
    """Shared state passed through the active operation workflow graph."""
