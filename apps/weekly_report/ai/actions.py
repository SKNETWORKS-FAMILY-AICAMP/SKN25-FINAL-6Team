...
# 마케팅 시사점을 반드시 1개 이상 포함하도록 지시하는 이유:
# 기획팀의 주요 관심사는 운영 이슈와 함께 프로모션·캠페인 연계 가능성이므로,
# 시스템 프롬프트에 강제 조건으로 명시한다.
_SYSTEM_PROMPT = (
...
def _llm_available() -> bool:
    # 모델명과 API 키 둘 다 있어야 실제 호출이 가능하다.
    return bool(os.environ.get("LLM_MODEL") and os.environ.get("LLM_API_KEY"))


def _fallback(reason: str) -> dict[str, Any]:
    """LLM 호출 불가 또는 실패 시 운영자에게 안내 메시지를 반환한다.

    PDF·Slack 렌더러가 빈 actions 리스트에 대응하지 않아도 되도록 최소 1개 항목을 포함한다.
    """
    return {
...
def _build_user_prompt(report_payload: dict[str, Any]) -> str:
    """report_payload에서 핵심 수치를 추출해 LLM 입력용 텍스트 프롬프트를 구성한다.

    report_payload는 build_report_payload()가 조립한 형식이지만,
    Airflow에서 직접 ai_input dict를 넘기는 경우(summary.total_count 형식)도 지원한다.
    """
    summary = report_payload.get("summary", {})
    # build_report_payload 경로: analysis_count / 직접 호출 경로: total_count — 둘 다 허용.
    ...
    # anomaly_section은 구버전 페이로드 호환용이며, 없으면 spike_alerts의 hourly 목록에서 추출한다.
    ...
    # category_distribution은 list(build_report_payload 형식) 또는 dict(직접 호출 형식) 모두 처리한다.
    cat_dist_raw = report_payload.get("category_distribution", {})
    if isinstance(cat_dist_raw, list):
        # [{"label": "결제", "value": 12}, ...] 형식
        cat_dist_str = ", ".join(
...
def generate_ai_actions(report_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    반환값은 AiRecommendedActions.model_dump() 형식:
    {"headline": str, "actions": [{"rank": int, "category": str, "action": str, "reason": str}, ...]}
    LLM 오류 시 _fallback()과 동일한 구조를 반환해 호출부가 None을 처리하지 않아도 된다.
    """
    ...
    try:
        ...
        # 타임아웃, 파싱 실패, 네트워크 오류 등 모든 예외를 fallback으로 흡수한다.
        return _fallback(f"AI 권장 액션 호출에 실패했습니다: {exc}")

    # Pydantic 모델 → dict 변환 후 반환해 호출부가 타입을 신경 쓰지 않아도 된다.
