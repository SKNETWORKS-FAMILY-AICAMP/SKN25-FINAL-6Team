...
- `dashboard-frontend`: React / Vite dev server
- `nginx`: `/dashboard` 및 `/dashboard/api` 프록시

## Runtime Paths

- UI: `http://localhost/dashboard/`
- API health: `http://localhost/dashboard/api/health`

## Compose Notes

- compose는 각 앱 디렉터리를 그대로 마운트하는 개발형 구성입니다.
- 백엔드는 `/app/apps/dashboard/backend`를 working directory로 사용합니다.
- Python import를 위해 `PYTHONPATH=/app/packages/common-python/src:/app/apps/dashboard/backend`를 사용합니다.
- 프런트는 `VITE_DASHBOARD_API_BASE_URL=http://dashboard-backend:8000`로 API를 바라봅니다.
