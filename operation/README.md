...
- `step12agent`: LangGraph StateGraph(STEP1+STEP2) 구현 완료. `runners/run_operation.py`로 단독 실행 가능.
- `approvalagent`: LangChain LCEL chain(`load → alignment → safety → decision`) 구현 완료. `runners/run_approval.py`로 단독 실행 가능.
- `runners/run_pipeline.py`: step12agent 출력을 approvalagent 입력으로 연결하는 통합 파이프라인 러너 구현 완료.
- middleware, retry, fallback은 아직 문서 수준의 권장안이며 실제 연결은 남아 있다.

즉 이 디렉터리는 전체 설계 기준과 payload 구조는 고정되어 있고, STEP1~Approval Gate 실행 로직은 완료된 상태다.
...
- 실행 진입점: `runners/run_operation.py` (STEP1+STEP2 단독), `runners/run_approval.py` (Approval Gate 단독), `runners/run_pipeline.py` (전체 파이프라인)
